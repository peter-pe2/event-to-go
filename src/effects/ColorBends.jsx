...
void main() {
  ...
  vec2 q = vec2(rp.x * (uCanvas.x / uCanvas.y), rp.y + (uScroll / uCanvas.y) * 2.0);
  q /= max(uScale, 0.0001);
  q /= 0.5 + 0.2 * dot(q, q);
  q += 0.2 * cos(t) - 7.56;
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      scrollCurrentRef.current += (scrollTargetRef.current - scrollCurrentRef.current) * Math.min(1, dt * 10);
      material.uniforms.uScroll.value = scrollCurrentRef.current;
